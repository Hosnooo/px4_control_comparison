...
bool isKnownKey(const std::string &key) {
  return std::find(kRequiredKeys.begin(), kRequiredKeys.end(), key) !=
         kRequiredKeys.end();
}
...
std::array<double, 5> parseCoefficients(const std::string &text,
...
HardwareWrenchCalibration HardwareWrenchCalibration::loadFromText(
  ...
  if (limits.minimum_sample_count == 0 ||
      !(limits.maximum_condition_number > 1.0) ||
      !(limits.maximum_collective_rmse_n >= 0.0) ||
      !limits.maximum_moment_rmse_nm.finite() ||
      limits.maximum_moment_rmse_nm.x < 0.0 ||
      limits.maximum_moment_rmse_nm.y < 0.0 ||
      limits.maximum_moment_rmse_nm.z < 0.0 ||
      !(limits.maximum_collective_residual_n >= 0.0) ||
      !limits.maximum_moment_residual_nm.finite() ||
      limits.maximum_moment_residual_nm.x < 0.0 ||
      limits.maximum_moment_residual_nm.y < 0.0 ||
      limits.maximum_moment_residual_nm.z < 0.0 || limits.maximum_age_days < 0 ||
      !limits.maximum_reconstruction_error_nm.finite() ||
  ...
  if (fields.at("schema_version") != "1" ||
      fields.at("authority") != "measured_hardware") {
  ...
  if (sha.size() != 64 ||
      !std::all_of(sha.begin(), sha.end(), [](unsigned char character) {
  ...
  const int recorded_minimum_samples = parseInt(
      fields.at("accept_minimum_sample_count"), "accept_minimum_sample_count");
  const double recorded_maximum_condition = parseDouble(
      fields.at("accept_maximum_condition_number"), "accept_maximum_condition_number");
  ...
  const double collective_rmse =
      parseDouble(fields.at("rmse_collective_n"), "rmse_collective_n");
  const Vec3 moment_rmse{
      parseDouble(fields.at("rmse_moment_x_nm"), "rmse_moment_x_nm"),
      parseDouble(fields.at("rmse_moment_y_nm"), "rmse_moment_y_nm"),
      parseDouble(fields.at("rmse_moment_z_nm"), "rmse_moment_z_nm")};
  const double collective_residual = parseDouble(
      fields.at("max_residual_collective_n"), "max_residual_collective_n");
  const Vec3 moment_residual{
      parseDouble(fields.at("max_residual_moment_x_nm"),
                  "max_residual_moment_x_nm"),
      parseDouble(fields.at("max_residual_moment_y_nm"),
                  "max_residual_moment_y_nm"),
      parseDouble(fields.at("max_residual_moment_z_nm"),
                  "max_residual_moment_z_nm")};
  if (collective_rmse < 0.0 || moment_rmse.x < 0.0 || moment_rmse.y < 0.0 ||
      moment_rmse.z < 0.0 || collective_residual < 0.0 ||
      moment_residual.x < 0.0 || moment_residual.y < 0.0 ||
      moment_residual.z < 0.0 ||
      collective_rmse >
          std::min(recorded_maximum_collective_rmse, limits.maximum_collective_rmse_n) ||
      moment_rmse.x >
          std::min(recorded_maximum_moment_rmse, limits.maximum_moment_rmse_nm.x) ||
      moment_rmse.y >
          std::min(recorded_maximum_moment_rmse, limits.maximum_moment_rmse_nm.y) ||
      moment_rmse.z >
          std::min(recorded_maximum_moment_rmse, limits.maximum_moment_rmse_nm.z) ||
      collective_residual >
          std::min(recorded_maximum_collective_residual,
                   limits.maximum_collective_residual_n) ||
      moment_residual.x >
          std::min(recorded_maximum_moment_residual,
  ...
  calibration.torque_min_ = {
      parseDouble(fields.at("torque_x_min"), "torque_x_min"),
      parseDouble(fields.at("torque_y_min"), "torque_y_min"),
      parseDouble(fields.at("torque_z_min"), "torque_z_min")};
  calibration.torque_max_ = {
      parseDouble(fields.at("torque_x_max"), "torque_x_max"),
      parseDouble(fields.at("torque_y_max"), "torque_y_max"),
      parseDouble(fields.at("torque_z_max"), "torque_z_max")};
  if (!(calibration.thrust_min_ >= 0.0) ||
      !(calibration.thrust_min_ < calibration.thrust_max_) ||
      calibration.thrust_max_ > 1.0 ||
      !(calibration.torque_min_.x < calibration.torque_max_.x) ||
  ...
  if (!std::isfinite(torque_condition) ||
      torque_condition > limits.maximum_condition_number) {
...
HardwareWrenchResult HardwareWrenchCalibration::normalize(
  ...
  if (normalized_collective_thrust < thrust_min_ ||
      normalized_collective_thrust > thrust_max_) {
  ...
  for (std::size_t row = 0; row < 3; ++row) {
    moment_without_bias[row] -= coefficients_[row + 1][0] +
                                coefficients_[row + 1][1] * normalized_collective_thrust;
  }
  ...
  if (!torque.finite() || torque.x < torque_min_.x || torque.x > torque_max_.x ||
      torque.y < torque_min_.y || torque.y > torque_max_.y ||
      torque.z < torque_min_.z || torque.z > torque_max_.z) {
    result.status = HardwareWrenchStatus::outside_calibrated_range;
    result.reason = "normalized torque is outside the measured range";
    return result;
  }

  const std::array<double, 5> input{1.0, normalized_collective_thrust,
