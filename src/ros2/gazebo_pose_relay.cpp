...
GazeboDirectPoseMessageResult makeGazeboDirectPose(const gz::msgs::Pose_V &poses,
  ...
  if (!selected.ok || selected.source_pose_index >= static_cast<std::size_t>(poses.pose_size())) {
    result.reason = selected.ok ? "Gazebo pose selection index out of range" : selected.reason;
    return result;
  }
