...
struct F450WrenchConfig {
  ...
  double collective_force_tolerance_n{};
...
struct F450WrenchResult {
  ...
  std::array<double, 4> rotor_speed_radps{};
  std::array<double, 4> rotor_thrust_n{};
  double reconstructed_collective_thrust_n{};
  Vec3 reconstructed_body_moment_frd_nm{};
  double collective_force_residual_n{};
  int iterations{};
};

F450WrenchConfig frozenF450WrenchConfig();

/** Frozen F450 simulation normalizer that forward-reconstructs the requested physical wrench. */
class F450WrenchModel {
 public:
  ...
  F450WrenchResult normalize(double desired_collective_force_n,
