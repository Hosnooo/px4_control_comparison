...
using namespace px4_offboard::vehicles::f450;
...
int main() {
  ...
  checkArrayNear(positive_roll.motor_command,
                 {0.592928932188, 0.607071067812, 0.607071067812, 0.592928932188}, 1e-12,
                 "PX4 normalized roll allocation");
  checkVecNear(positive_roll.body_moment_frd_nm, {0.061934399999, 0.0, 0.0}, 1e-10,
               "positive physical roll moment");

  const auto positive_pitch = model.forward({0.0, 0.01, 0.0}, 0.60);
  check(positive_pitch.ok, positive_pitch.reason);
  checkVecNear(positive_pitch.body_moment_frd_nm, {0.0, 0.061934399999, 0.0}, 1e-10,
               "positive physical pitch moment");

  const auto positive_yaw = model.forward({0.0, 0.0, 0.01}, 0.60);
  check(positive_yaw.ok, positive_yaw.reason);
  checkVecNear(positive_yaw.body_moment_frd_nm, {0.0, 0.0, 0.007378272}, 1e-12,
               "positive physical yaw moment");
  ...
  const auto combined =
      model.normalize(combined_forward.collective_thrust_n, combined_moment, 0.60);
  ...
  checkNear(combined.reconstructed_collective_thrust_n, combined_forward.collective_thrust_n,
            1e-9, "combined physical collective reconstruction");
  ...
  const auto invalid_force =
      model.normalize(std::numeric_limits<double>::quiet_NaN(), {}, 0.60);
  ...
  const auto iteration_limited = F450WrenchModel(iteration_limited_config)
                                     .normalize(20.0, combined_moment, 0.60);
