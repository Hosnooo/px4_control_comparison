...
COLUMNS = [
    "normalized_thrust", "torque_x", "torque_y", "torque_z",
    ...
    def test_exact_affine_fit_writes_loader_record(self):
        ...
        for field, expected in zip(
                ["coefficient_collective", "coefficient_moment_x",
                 "coefficient_moment_y", "coefficient_moment_z"], COEFFICIENTS):
        ...
        runner_source.write_text(r'''#include "px4_offboard_controllers/calibration/hardware_wrench_calibration.hpp"
#include <cmath>
using namespace px4_offboard;
using namespace px4_offboard::calibration;
int main(int argc, char **argv) {
  if (argc != 2) return 1;
  const HardwareCalibrationLimits limits{
      50, 100.0, 1e-9, {1e-9, 1e-9, 1e-9},
      1e-8, {1e-8, 1e-8, 1e-8}, 30, {1e-10, 1e-10, 1e-10}};
  const auto calibration = HardwareWrenchCalibration::loadFromFile(
      argv[1], "f450_lab_01", "2026-09-10", limits);
  const auto result = calibration.normalize(19.0015, {0.121, -0.129, 0.018}, 0.60);
...
''', encoding="utf-8")
        compile_result = subprocess.run([
            ...
            f"-I{ROOT / 'include'}", f"-I{ROOT / 'experiment' / 'calibration' / 'include'}",
            str(ROOT / "experiment" / "calibration" / "src" /
                "hardware_wrench_calibration.cpp"), str(runner_source), "-o", str(runner_binary),
    ...
    def test_rejects_invalid_datasets_and_quality(self):
        ...
        noisy[0][4] += 0.1
        completed, _ = self.run_fitter(noisy, maximum_rmse=1e-4, maximum_residual=1e-4)
