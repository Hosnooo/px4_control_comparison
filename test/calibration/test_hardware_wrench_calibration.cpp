...
int main() {
  ...
  checkVecNear(result.reconstructed_body_moment_frd_nm, {0.121, -0.129, 0.018}, 1e-12,
  ...
  const auto force_mismatch = calibration.normalize(18.0, {0.121, -0.129, 0.018}, 0.60);
  ...
  expectReject(replaced(kValidRecord, "authority=measured_hardware", "authority=simulation"),
               "simulation authority");
  expectReject(replaced(kValidRecord, "vehicle_id=f450_lab_01", "vehicle_id=another_vehicle"),
               "wrong vehicle");
  expectReject(replaced(kValidRecord, "output_units=N_Nm", "output_units=SI"), "wrong units");
  ...
  expectReject(replaced(kValidRecord, "rmse_collective_n=0.02", "rmse_collective_n=0.2"),
               "excessive fit RMSE");
  expectReject(replaced(kValidRecord, "rmse_collective_n=0.02", "rmse_collective_n=-0.02"),
               "negative fit metric");
