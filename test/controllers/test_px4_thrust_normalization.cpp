...
int main() {
  ...
  const auto downward_acceleration =
      normalizer.fromAccelerationSetpoint({0.0, 0.0, 2.0});
  ...
  const auto tilt_limited = Px4ThrustNormalization(limited_config)
                                .fromAccelerationSetpoint({20.0, 0.0, 0.0});
  const double tilt_rad =
      std::acos(std::clamp(tilt_limited.desired_body_z_ned.z, -1.0, 1.0));
  ...
  checkVecNear(saturated.desired_body_z_ned, normalized(-saturated.normalized_thrust_ned), 1e-12,
               "attitude direction follows PX4 final saturated thrust vector");
