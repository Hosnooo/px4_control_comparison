...
int main() {
  ...
  checkVecNear(controller.update(identity, identity, {}).body_rate_setpoint_frd_radps, {}, 1e-12,
               "zero error");
  ...
  check(std::abs(limited.body_rate_setpoint_frd_radps.z) <= config.rate_limit_radps.z + 1e-12,
        "yaw rate limited");
